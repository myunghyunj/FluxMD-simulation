...
def process_flux_iterations(output_dir, protein_file, protein_name="protein"):
                ...
                converted_df = pd.DataFrame(
                    {
                        ...
                        "is_aromatic": df["protein_resname"]
                        .isin(["PHE", "TYR", "TRP", "HIS"])
                        .astype(int),
                    }
                )
    ...
    for res_id, group in grouped:
        stats = {
            ...
            "residue_name": (
                group["residue_name"].iloc[0] if "residue_name" in group.columns else f"RES{res_id}"
            ),
        ...
        stats["ci_lower"] = stats["average_flux"] - 1.96 * stats["std_flux"] / np.sqrt(
            stats["n_observations"]
        )
        stats["ci_upper"] = stats["average_flux"] + 1.96 * stats["std_flux"] / np.sqrt(
            stats["n_observations"]
        )
    ...
    for i, row in processed_df.head(20).iterrows():
        print(
            f"{i+1:<5} {int(row['residue_index']):<10} {row['residue_name']:<5} "
    ...
    try:
        ...
        ax1.set_xticks(x)
        ax1.set_xticklabels(
            [f"{int(r['residue_index'])}\n{r['residue_name']}" for _, r in top_30.iterrows()],
            rotation=45,
            ha="right",
        )
    ...
    with open(report_file, "w") as f:
        ...
        f.write("-" * 60 + "\n")
        f.write(
            f"{'Rank':<5} {'Residue':<10} {'Name':<5} {'Avg Flux':<12} {'P-value':<10} {'Aromatic':<10}\n"
        )
        f.write("-" * 60 + "\n")
        ...
        if "is_aromatic" in processed_df.columns:
            ...
            if len(aromatic_res) > 0:
                ...
                f.write(
                    f"Average flux of aromatic residues: {aromatic_res['average_flux'].mean():.6f}\n"
                )
...
def main():
    if len(sys.argv) < 3:
        print(
            "Usage: python process_completed_iterations.py <output_dir> <protein_pdb> [protein_name]"
        )
        sys.exit(1)
