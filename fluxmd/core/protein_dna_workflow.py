...
def run_protein_dna_workflow(dna_file: str, protein_file: str, output_dir: str, **kwargs):
    ...
    gpu_calc = GPUAcceleratedInteractionCalculator(
        device=device, physiological_pH=physiological_pH, target_is_dna=True
    )
    ...
    dna_ca_coords = (
        dna_backbone[["x", "y", "z"]].values
        if not dna_backbone.empty
        else dna_atoms[["x", "y", "z"]].values[:100]
    )
    ...
    for iteration_num in range(n_iterations):
        ...
        for approach_num in range(n_approaches):
            ...
            print(
                f"   Initial distance: {initial_distance:.1f} Å (approach {approach_num + 1}/{n_approaches})"
            )
    ...
    results_df = pd.DataFrame(
        {
            ...
            "base_type": [
                flux_analyzer.base_types[i] if i < len(flux_analyzer.base_types) else "?"
                for i in range(len(flux_data["res_indices"]))
            ],
            "residue_name": [
                flux_analyzer.residue_names[i] if i < len(flux_analyzer.residue_names) else "?"
                for i in range(len(flux_data["res_indices"]))
            ],
            "mean_flux": flux_data["avg_flux"],
            "std_flux": flux_data["std_flux"],
            "p_value": flux_data.get("p_values", [0.05] * len(flux_data["res_indices"])),
            "significant": flux_data["avg_flux"]
            > np.percentile(flux_data["avg_flux"][flux_data["avg_flux"] > 0], 75),
        }
    )
...
def _write_dna_report(output_dir, dna_name, flux_data, flux_analyzer):
    ...
    with open(report_file, "w") as f:
        ...
        if hasattr(flux_analyzer, "base_types") and flux_analyzer.base_types:
            ...
            high_flux_positions = sorted(
                [
                    flux_analyzer.residue_indices[i]
        ...
        f.write(
            f"Mean flux: {np.mean(flux_data['avg_flux']):.4f} ± {np.std(flux_data['avg_flux']):.4f}\n"
        )
