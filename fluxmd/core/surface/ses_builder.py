...
class SurfaceMesh:
    ...
    def __init__(
        ...
        vertices: np.ndarray,
        faces: np.ndarray,
        groove_labels: Optional[Dict[int, str]] = None,
    ) -> None:
...
class SESBuilder:
    ...
    def build_distance_field(
        ...
        for start_idx in range(0, n_points, chunk_size):
            ...
            distances = np.linalg.norm(
                chunk_points[:, np.newaxis, :] - self.coords[np.newaxis, :, :], axis=2
            )
    ...
    def marching_cubes(self, distance_field: np.ndarray, iso_value: float) -> SurfaceMesh:
