...
class MatryoshkaLayerGenerator:
    ...
    def _smooth_problematic_regions(
        self, vertices: np.ndarray, faces: np.ndarray, problematic: np.ndarray, iterations: int = 3
    ) -> np.ndarray:
    ...
    def _offset_layer(self, layer_idx: int) -> SurfaceMesh:
        ...
        if np.any(problematic):
            ...
            offset_vertices = self._smooth_problematic_regions(offset_vertices, faces, problematic)
