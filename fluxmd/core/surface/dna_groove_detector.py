...
class DNAGrooveDetector:
    ...
    def _identify_dna_backbone(self) -> None:
        ...
        for i, (name, resname) in enumerate(zip(self.atoms['names'], 
                                               self.atoms.get('resnames', [''] * len(self.atoms['names'])))):
    ...
    def _calculate_groove_direction(self, point: np.ndarray, 
                                  backbone_indices: List[int]) -> Tuple[np.ndarray, str]:
    ...
    def label_surface_vertices(self, vertices: np.ndarray, 
                              radius_cutoff: float = 5.0) -> Dict[int, str]:
        ...
        for i, vertex in enumerate(vertices):
            ...
            distances, indices = self.backbone_tree.query(
                vertex, k=min(10, self.n_backbone), 
                distance_upper_bound=radius_cutoff
            )
        ...
        print(f"  Groove labeling: {major_count} major, {minor_count} minor, "
              f"{len(vertices) - major_count - minor_count} other")
        
        return vertex_labels
    
    def get_groove_bias(self, position: np.ndarray, target_position: np.ndarray,
                       groove_preference: str = 'major') -> float:
        ...
        distances, indices = self.backbone_tree.query(
            target_position, k=min(10, self.n_backbone), 
            distance_upper_bound=8.0
        )
        ...
        groove_dir, groove_type = self._calculate_groove_direction(
            target_position, valid_indices
        )
