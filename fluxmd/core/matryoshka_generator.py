...
class MatryoshkaTrajectoryGenerator:
    ...
    def __init__(
        ...
        try:
            ...
            print(
                f"Warning: Failed to parse n_workers={n_workers_param}, using auto-detection. Error: {e}"
            )
        ...
        self.anchors = extreme_calpha_pairs(
            self.protein_atoms["coords"], self.protein_atoms["names"]
        )
        ...
        self.layer_generator = MatryoshkaLayerGenerator(self.base_surface, step=self.layer_step)

        # Calculate maximum useful layers
        self.max_layers = self.layer_generator.get_max_useful_layers(
            ligand_radius=self.ligand_sphere["radius"], cutoff=self.params.get("vdw_cutoff", 12.0)
        )
    ...
    def _calculate_ligand_sphere(self) -> Dict[str, Any]:
        ...
        rgyr_squared = np.average(np.sum(centered_coords**2, axis=1), weights=masses)
    ...
    def _generate_orientation_variants(
        self, base_quaternion: np.ndarray, n_variants: int = 4
    ) -> np.ndarray:
    ...
    def _calculate_ref15_energy_batch(
        self, position: np.ndarray, quaternions: np.ndarray, sample_fraction: float = 0.1
    ) -> np.ndarray:
        ...
        for i, quat in enumerate(quaternions):
            ...
            protein_sample = {"coords": protein_coords_sample, "names": protein_names_sample}
    ...
    def _run_single_trajectory(
        self, layer_idx: int, iteration_idx: int, seed: Optional[int] = None
    ) -> Dict[str, Any]:
        ...
        if self.ref15_calculator and len(trajectory["pos"]) > 0:
            ...
            sample_indices = np.linspace(0, len(trajectory["pos"]) - 1, n_samples, dtype=int)
    ...
    def _worker_process(self, work_queue: mp.Queue, result_queue: mp.Queue, worker_id: int) -> None:
    ...
    def _save_checkpoint(
        self, trajectories: List[Dict[str, Any]], layer_idx: int, iteration_idx: int
    ) -> None:
    ...
    def run(
        self, n_layers: Optional[int] = None, n_iterations: int = 10
        ...
        if self.n_workers > 1 and total_items - completed_items > 1:
            # Parallel execution
            trajectories.extend(
                self._run_parallel(n_layers, n_iterations, start_layer, start_iteration)
            )
        else:
            # Serial execution
                ...
                for layer_idx in range(n_layers):
                    ...
                    for iteration_idx in range(n_iterations):
                        ...
                        if len(trajectories) % 5 == 0:
                            self._save_checkpoint(trajectories, layer_idx, iteration_idx + 1)
    ...
    def _run_parallel(
        self, n_layers: int, n_iterations: int, start_layer: int = 0, start_iteration: int = 0
    ) -> List[Dict[str, Any]]:
        ...
        for i in range(self.n_workers):
            p = mp.Process(target=self._worker_process, args=(work_queue, result_queue, i))
            p.start()
            workers.append(p)
