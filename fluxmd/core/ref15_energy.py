...
from .rosetta_atom_types import get_atom_typer, AtomTypeInfo
from .ref15_params import get_ref15_params, switch_function, fade_function, sigmoidal_dielectric
...
class REF15EnergyCalculator:
    ...
    def _calculate_lj_energy(self, atom1: AtomContext, atom2: AtomContext, 
        ...
        if x < 0.6:
            ...
            if x < 2.0:  # REF15 attractive cutoff
                ...
                switch = switch_function(distance, switch_params['switch_dis'], 
                                       switch_params['cutoff_dis'])
    ...
    def _calculate_solvation_energy(self, atom1: AtomContext, atom2: AtomContext,
    ...
    def _calculate_electrostatic_energy(self, atom1: AtomContext, atom2: AtomContext,
                                       distance: float) -> float:
        """
    ...
    def _can_form_hbond(self, atom1: AtomContext, atom2: AtomContext) -> bool:
        ...
        return ((atom1.is_donor and atom2.is_acceptor) or 
                (atom2.is_donor and atom1.is_acceptor))
                
    def _calculate_hbond_energy_simple(self, atom1: AtomContext, atom2: AtomContext,
                                      distance: float) -> float:
        """
        ...
        # Evaluate polynomial
        r = distance
        e_dist = poly[0] + poly[1]*r + poly[2]*r*r + poly[3]*r*r*r + poly[4]*r*r*r*r
    ...
    def calculate_hbond_energy_full(self, donor_heavy: AtomContext, hydrogen: AtomContext,
                                   acceptor: AtomContext, acceptor_base: Optional[AtomContext] = None) -> float:
        """
        ...
        # Distance component
        e_dist = poly[0] + poly[1]*r_ha + poly[2]*r_ha*r_ha + \
                 poly[3]*r_ha*r_ha*r_ha + poly[4]*r_ha*r_ha*r_ha*r_ha
        ...
        if donor_heavy.is_backbone and acceptor.is_backbone:
            weight_key = 'hbond_sr_bb' if abs(donor_heavy.residue_id - acceptor.residue_id) <= 4 else 'hbond_lr_bb'
