...
class IntelligentCocoonSampler:
    ...
    def _calculate_electrostatic_surface(self):
        ...
        self.elec_grid_points = (
            np.array(self.elec_grid_points) if self.elec_grid_points else np.array([])
        )
        self.elec_potentials = (
            np.array(self.elec_potentials) if self.elec_potentials else np.array([])
        )
    ...
    def _identify_hydrophobic_regions(self):
        ...
        for atom in self.protein_atoms:
            if (
                atom.get("element", "C") in hydrophobic_atoms
                and atom.get("resname", "") in hydrophobic_residues
            ):
    ...
    def _map_hbond_sites(self):
        ...
        logger.info(
            f"  Found {len(self.hbond_donors)} H-bond donors, {len(self.hbond_acceptors)} acceptors"
        )
    ...
    def _find_aromatic_patches(self):
        ...
        for (resname, resid, chain), atoms in residue_atoms.items():
            ...
            if len(ring_coords) >= 3:
                ...
                self.aromatic_centers.append({"coord": center, "residue": resname, "resid": resid})

        logger.info(f"  Found {len(self.aromatic_centers)} aromatic rings")

    def generate_approach_points(
        self,
        n_trajectories: int,
        ligand_charge: float = 0.0,
        ligand_has_aromatic: bool = False,
        ligand_has_hbond: bool = True,
    ) -> List[np.ndarray]:
        """
        ...
        logger.info(
            f"  Ligand properties: charge={ligand_charge:.1f}, aromatic={ligand_has_aromatic}, H-bond={ligand_has_hbond}"
        )
    ...
    def _allocate_trajectory_types(
        self, n_total: int, ligand_charge: float, has_aromatic: bool, has_hbond: bool
    ) -> Dict[str, int]:
        """Intelligently allocate trajectory types based on ligand properties"""
        allocations = {"electrostatic": 0, "hbond": 0, "hydrophobic": 0, "aromatic": 0, "random": 0}
