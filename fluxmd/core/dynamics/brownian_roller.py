...
class BrownianSurfaceRoller:
    ...
    def _find_closest_point_on_surface_mesh(self, point: np.ndarray, 
                                           mesh: SurfaceMesh) -> Tuple[np.ndarray, float, np.ndarray]:
    ...
    def _find_closest_point_on_surface(self, point: np.ndarray) -> Tuple[np.ndarray, float, np.ndarray]:
    ...
    def _guidance_force(self, position: np.ndarray, path_length: float, current_time: float, total_length: float | None = None) -> np.ndarray:
    ...
    def run(
        ...
        for step in range(max_steps):
            ...
            angular_velocity = self.ou_decay_rot * angular_velocity + self.ou_noise_rot * self.rng.normal(size=3)
            ...
            if (step > 0 and step % self.hop_attempt_interval == 0 and 
                self.rng.random() < self.hop_probability):
        ...
        print(f"Layer hops: {self.successful_hops}/{self.hop_attempts} " +
