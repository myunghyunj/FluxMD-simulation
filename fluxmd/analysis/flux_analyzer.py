...
class TrajectoryFluxAnalyzer:
    ...
    def parse_pdb_for_ribbon(self, filepath):
        ...
        self.atom_to_residue_map, self.residue_to_atom_map = self.create_atom_to_residue_mapping(
            structure
        )
        ...
        if "protein_residue_id" not in df.columns:
            ...
            for _, row in df.iterrows():
                if "centroid1_x" in row:
                    centroid = np.array(
                        [row["centroid1_x"], row["centroid1_y"], row["centroid1_z"]]
                    )
                    ...
                    if "Pi-Stacking" in str(row.get("bond_type", "")):
                        ...
                        if "protein_residue_id" in row and row["protein_residue_id"] != -1:
                            ...
                            if all(
                                ...
                                centroid2 = np.array(
                                    [row["centroid2_x"], row["centroid2_y"], row["centroid2_z"]]
                                )
                                angle = row.get("angle", 0)
                                offset = row.get(
                                    "offset_distance", np.linalg.norm(centroid2 - centroid1)
                                )
                                ...
                                vector = np.array(
                                    [row["vector_x"], row["vector_y"], row["vector_z"]]
                                )
    ...
    def process_gpu_trajectory_results(self, gpu_trajectory_results, residue_indices):
        ...
        for frame_idx, frame_result in enumerate(gpu_trajectory_results):
            ...
            if (
                hasattr(interactions, "combined_vectors")
                and interactions.combined_vectors is not None
            ):
                ...
                vectors = np.column_stack(
                    [energies, np.zeros_like(energies), np.zeros_like(energies)]
                )
    ...
    def calculate_tensor_flux_differentials(
        self, residue_tensors, ca_coords, residue_indices, residue_raw_vectors=None
    ):
            ...
            if res_id in residue_tensors:
                ...
                if len(tensor) > 3:
                    try:
                            ...
                            if len(raw_vectors) > 1:
                                ...
                                if sum_magnitudes > 1e-10:
                                    directional_consistency = (
                                        np.linalg.norm(sum_vectors) / sum_magnitudes
                                    )
                                else:
                        ...
                        flux_differentials[i] = (
                            mean_magnitude * directional_consistency * (1 + rate_of_change)
                        )
    ...
    def create_ultra_smooth_backbone(self, ca_coords, flux_values, smoothing_factor=10):
        ...
        flux_interp = interp1d(
            arc_length, flux_values, kind="cubic", bounds_error=False, fill_value="extrapolate"
        )
    ...
    def process_trajectory_iterations(self, results_dir, protein_pdb, gpu_trajectory_results=None):
        ...
        if gpu_trajectory_results is not None:
            ...
            if isinstance(gpu_trajectory_results, list) and len(gpu_trajectory_results) > 0:
                ...
                residue_tensors = self.process_gpu_trajectory_results(
                    gpu_trajectory_results, res_indices
                )
            ...
            for idx, iter_dir in enumerate(iter_dirs):
                ...
                try:
                    residue_tensors, residue_raw_vectors = self.process_iteration_files(
                        iter_dir, "flux_iteration_*_output_vectors.csv", structure_file=protein_pdb
                    )
    ...
    def visualize_trajectory_flux(self, flux_data, protein_name, output_dir):
        ...
        if "bootstrap_stats" in flux_data:
            ci_lower = np.array(
                [flux_data["bootstrap_stats"][res_id]["ci_lower"] for res_id in res_indices]
            )
            ci_upper = np.array(
                [flux_data["bootstrap_stats"][res_id]["ci_upper"] for res_id in res_indices]
            )
        else:
        ...
        ax1.fill_between(res_indices, ci_lower, ci_upper, alpha=0.3, color="blue", label="95% CI")
        ...
        ax1.scatter(
            high_flux_indices, high_flux_values, color="red", s=50, zorder=5, label="High Flux"
        )
        ...
        ax2.scatter(
            high_flux_indices, high_flux_absolute, color="red", s=50, zorder=5, label="High Flux"
        )
        ...
        if "bootstrap_stats" in flux_data:
            ...
            for res_id in res_indices:
                if res_id in flux_data["bootstrap_stats"]:
                    ...
                    ci_lower.append(
                        flux_data["average_flux"][list(res_indices).index(res_id)]
...
class BootstrapFluxValidator:
    ...
    def bootstrap_flux_analysis(self, all_flux_data, residue_indices, n_bootstrap=None):
        ...
        print(
            f"   ✓ Bootstrap complete: {n_significant}/{n_residues} residues significant (p<0.05)"
        )
...
def main():
    ...
    analyzer = TrajectoryFluxAnalyzer(
        output_dir="output_directory", protein_name="GPX4", target_is_dna=False
    )
