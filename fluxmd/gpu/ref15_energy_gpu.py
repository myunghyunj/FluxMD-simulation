...
class REF15GPUCalculator:
    ...
    def _build_parameter_matrices(self):
        ...
        # Weights
        self.weights = {
            k: torch.tensor(v, device=self.device) for k, v in self.params.weights.items()
        }
    ...
    def calculate_energy_batch(
        self, protein_arrays: REF15GPUArrays, ligand_arrays: REF15GPUArrays, cutoff: float = 6.0
        ...
        e_sol = self._calculate_solvation_batch(
            p_indices, l_indices, distances, protein_arrays, ligand_arrays
        )
    ...
    def _calculate_lj_batch(
        self, type1_idx: torch.Tensor, type2_idx: torch.Tensor, distances: torch.Tensor
    ...
    def _calculate_solvation_batch(
        self,
        p_indices: torch.Tensor,
        l_indices: torch.Tensor,
        distances: torch.Tensor,
        p_arrays: REF15GPUArrays,
        l_arrays: REF15GPUArrays,
    ) -> torch.Tensor:
        ...
        # Solvation energy
        e_sol = dgfree_p * burial_p * gaussian_l * (
            volume_l / (volume_p + 1e-6)
    ...
    def _calculate_electrostatics_batch(
        self, charges1: torch.Tensor, charges2: torch.Tensor, distances: torch.Tensor
    ) -> torch.Tensor:
        ...
        e_elec[charge_mask] = (
            332.0637
            * charges1[charge_mask]
            * charges2[charge_mask]
    ...
    def _switch_function(
        self, distance: torch.Tensor, switch_dis: float, cutoff_dis: float
    ) -> torch.Tensor:
    ...
    def get_residue_energies(
        self, energy_components: Dict[str, torch.Tensor], protein_arrays: REF15GPUArrays
    ) -> Dict[int, float]:
