...
class ProtonationAwareInteractionDetector:
    ...
    def detect_all_interactions(
        ...
        if distance <= self.cutoffs["hbond"]:
            ...
            if p_atom.can_donate_hbond and l_atom.can_accept_hbond:
                ...
                interactions.append(
                    {"type": "HBond", "energy": energy, "donor": "protein", "acceptor": "ligand"}
                )
            ...
            if l_atom.can_donate_hbond and p_atom.can_accept_hbond:
                ...
                if p_atom.formal_charge != 0 or l_atom.formal_charge != 0:
                    energy *= 1.5
                interactions.append(
                    {"type": "HBond", "energy": energy, "donor": "ligand", "acceptor": "protein"}
                )
        ...
        if distance <= self.cutoffs["salt_bridge"]:
            if p_atom.formal_charge * l_atom.formal_charge < 0:
                energy = self._calculate_salt_bridge_energy(
                    p_atom.formal_charge, l_atom.formal_charge, distance
                )
                interactions.append({"type": "Salt Bridge", "energy": energy})
    ...
    def _calculate_salt_bridge_energy(
        self, charge1: float, charge2: float, distance: float, epsilon_r: float = 80.0
    ) -> float:
