...
class SignedFluxAnalyzer:
    ...
    def calculate_signed_flux(self, interactions_df: pd.DataFrame) -> Dict[int, Dict[str, float]]:
        ...
        for res_id, group in interactions_df.groupby('protein_residue_id'):
            ...
            results[res_id] = {
                ...
                'attractive_fraction': np.sum(energies < 0) / len(energies) if len(energies) > 0 else 0
            }
    ...
    def analyze_trajectory(self, trajectory_dir: str, protein_name: str = "Protein") -> Dict:
        ...
        residue_stats = results_df.groupby('residue_id').agg({
    ...
    def visualize_signed_flux(self, analysis_results: Dict, save_path: Optional[str] = None):
        ...
        bars = ax1.bar(range(len(residue_ids)), mean_flux, 
                       yerr=std_flux, capsize=2, width=0.8)
        ...
        ax2.fill_between(range(len(cumulative_binding)), 0, cumulative_binding,
                        where=(cumulative_binding < 0), alpha=0.3, color='blue',
        ...
        print(f"Attractive residues: {attractive_residues} ({attractive_residues/len(residue_ids)*100:.1f}%)")
        print(f"Repulsive residues: {repulsive_residues} ({repulsive_residues/len(residue_ids)*100:.1f}%)")
        ...
        for idx, row in residue_stats.nsmallest(10, ('signed_flux', 'mean')).iterrows():
            ...
            print(f"Residue {idx}: Flux = {flux:.3f}, Mean Energy = {energy:.3f} kcal/mol, N = {n_int}")
        
        return fig


def analyze_case_study(trajectory_dir: str, protein_name: str = "GPX4", 
                      output_dir: str = "signed_flux_analysis"):
    """
