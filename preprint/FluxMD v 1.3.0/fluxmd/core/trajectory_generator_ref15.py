...
class REF15TrajectoryGenerator(CocoonTrajectoryGenerator):
    ...
    def __init__(self, protein_file: str, ligand_file: str, output_dir: str, 
                 target_is_dna: bool = False):
        # Initialize parent with REF15 energy function
        super().__init__(protein_file, ligand_file, output_dir, target_is_dna, 
    ...
    def generate_approach_trajectories_intelligent(self, n_trajectories: int) -> List[np.ndarray]:
        ...
        if self.intelligent_sampler is None:
            ...
            self.intelligent_sampler = IntelligentCocoonSampler(
                self.protein_coords,
                protein_atoms.to_dict('records'),
                self.ref15_calculator
            )
        ...
        approach_points = self.intelligent_sampler.generate_approach_points(
            ...
            ligand_has_hbond=(self.ligand_properties['has_hbond_donor'] or 
                             self.ligand_properties['has_hbond_acceptor'])
        )
    ...
    def run_enhanced_workflow(self, n_iterations: int = 100):
        ...
        for i, start_point in enumerate(approach_points):
            ...
            for frame_idx, position in enumerate(trajectory):
                ...
                ligand_coords_moved = self.move_ligand_to_position(
                    self.ligand_coords, position
                )
                
                # Calculate REF15 interactions
                interactions = self.calculate_ref15_interactions(
                    protein_atoms, ligand_atoms, ligand_coords_moved,
                    iteration_num=i, frame_num=frame_idx
                )
    ...
    def generate_single_trajectory(self, start_point: np.ndarray,
    ...
    def calculate_ref15_interactions(self, protein_atoms: pd.DataFrame,
                                   ligand_atoms: pd.DataFrame,
                                   ligand_coords: np.ndarray,
                                   iteration_num: int,
                                   frame_num: int) -> pd.DataFrame:
    ...
    def move_ligand_to_position(self, ligand_coords: np.ndarray, 
...
def create_ref15_trajectory_generator(protein_file: str, ligand_file: str, 
                                    output_dir: str, target_is_dna: bool = False) -> REF15TrajectoryGenerator:
