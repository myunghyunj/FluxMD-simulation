...
def save_iteration_data_uma(
    ...
    if flux_output_data:
        ...
        flux_csv_path = os.path.join(
            iter_dir, f"flux_iteration_{iteration_num + 1}_output_vectors.csv"
        )
    ...
    for approach_num in range(n_approaches):
        trajectory_csv_path = os.path.join(
            iter_dir, f"trajectory_iteration_{iteration_num + 1}_approach_{approach_num}.csv"
        )
...
def run_single_iteration_uma(
    ...
    for approach_num in range(n_approaches):
        ...
        if save_trajectories:
            ...
            if hasattr(self, "visualize_trajectory_cocoon"):
                self.visualize_trajectory_cocoon(
                    protein_atoms_df, trajectory, iteration_num, approach_num, iter_dir
                )
        ...
        if gpu_calc is not None:
            ...
            print(
                f"     Processing {len(trajectory)} frames × {n_rotations} rotations = {len(trajectory) * n_rotations} configurations"
            )
    ...
    if iteration_results:
        ...
        with open(summary_file, "w") as f:
            ...
            if total_frames > 0:
                f.write(
                    f"Average interactions per frame: {total_interactions / total_frames:.1f}\n"
                )
            f.write("\n")
            ...
            if all_energies:
                ...
                for result in iteration_results:
                    if (
                        result is not None
                        and hasattr(result, "interaction_types")
                        and len(result.interaction_types) > 0
                    ):
                        types = result.interaction_types.cpu().numpy()
                ...
                if total_typed_file > 0:
                    ...
                    for itype, count in interaction_type_counts_file.items():
                        ...
                        f.write(
                            f"  {InteractionResult.get_interaction_name(itype)}: {count} ({percentage:.1f}%)\n"
                        )
                ...
                if all_distances:
                    ...
                    f.write(
                        f"  Distances < 3.5Å (H-bond range): {np.sum(distances_array < 3.5)} ({100 * np.sum(distances_array < 3.5) / len(distances_array):.1f}%)\n"
                    )
                    f.write(
                        f"  Distances < 5.0Å (salt bridge range): {np.sum(distances_array < 5.0)} ({100 * np.sum(distances_array < 5.0) / len(distances_array):.1f}%)\n"
                    )
                else:
            ...
            if result is not None and len(result.energies) > 0:
                ...
                if hasattr(result, "interaction_types") and len(result.interaction_types) > 0:
                    ...
                    for res_id, energy in zip(residue_ids, energies):
                        trajectory_data.append(
                            {"frame": frame_idx, "residue_id": res_id, "energy": energy}
                        )
    ...
    for i in range(n_iterations):
        ...
        if iteration_results and total_interactions > 0:
            ...
            if save_trajectories:
                ...
                from ..analysis.flux_analyzer_uma import (
                    TrajectoryFluxAnalyzer as TrajectoryFluxAnalyzerUMA,
                )
                ...
                try:
                    ...
                    iter_flux_data = iter_flux_analyzer.process_iterations_and_calculate_flux(
                        single_iter_results, gpu_calc.intra_protein_vectors_gpu
                    )
