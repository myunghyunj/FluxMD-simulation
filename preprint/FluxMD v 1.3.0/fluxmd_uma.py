...
def main():
  ...
  fluxmd-uma protein.pdb ligand.pdb --save-trajectories --interaction-details
    ...
    parser.add_argument(
        "-o",
        "--output",
        default="flux_analysis_uma",
        help="Output directory (default: flux_analysis_uma)",
    )
    parser.add_argument(
        "-p", "--params", help="Load parameters from simulation_parameters.txt file"
    )
    parser.add_argument(
        "-s", "--steps", type=int, default=200, help="Number of steps per trajectory (default: 200)"
    )
    parser.add_argument(
        "-i", "--iterations", type=int, default=10, help="Number of iterations (default: 10)"
    )
    parser.add_argument(
        "-a", "--approaches", type=int, default=10, help="Number of approach angles (default: 10)"
    )
    parser.add_argument(
        "-d",
        "--distance",
    ...
    parser.add_argument("--ph", type=float, default=7.4, help="Physiological pH (default: 7.4)")
    parser.add_argument("--cpu", action="store_true", help="Force CPU usage (disable GPU)")
    parser.add_argument(
        "--save-trajectories", action="store_true", help="Save trajectory visualization images"
    )
    parser.add_argument(
        "--interaction-details", action="store_true", help="Show detailed interaction breakdown"
    )
    ...
    analyzer = ProteinLigandFluxAnalyzer(
        args.protein, args.ligand, args.output, target_is_dna=False
    )
    ...
    try:
        ...
        if flux_data is not None:
            ...
            for i, idx in enumerate(sorted_idx):
                if avg_flux[idx] > 0:
                    print(
                        f"  {i + 1}. Residue {res_indices[idx]} ({res_names[idx]}): {avg_flux[idx]:.4f}"
                    )
        ...
        print(f"\n📊 Results Summary:")
        print(
            f"   - Simulation parameters: {os.path.join(args.output, 'simulation_parameters.txt')}"
        )
        print(f"   - Flux analysis: {os.path.join(args.output, 'processed_flux_data.csv')}")
        print(
            f"   - Statistical report: {os.path.join(args.output, os.path.basename(args.protein).split('.')[0] + '_flux_report.txt')}"
        )
        print(
            f"   - Heatmap visualization: {os.path.join(args.output, os.path.basename(args.protein).split('.')[0] + '_trajectory_flux_analysis.png')}"
        )
