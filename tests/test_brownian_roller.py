...
def test_brownian_roller_init():
    ...
    roller = BrownianSurfaceRoller(
        surface=surface, ligand_sphere=ligand_sphere, anchors=anchors, T=298.15, viscosity=0.00089
    )
...
def test_adaptive_timestep():
    ...
    ligand_large = {"radius": 10.0, "mass": 5000.0, "center": np.array([0, 0, 15])}

    # Small light molecule - should have larger timestep
    ligand_small = {"radius": 1.0, "mass": 50.0, "center": np.array([0, 0, 2])}
...
def test_trajectory_termination():
    ...
    ligand_sphere = {"radius": 1.0, "mass": 100.0, "center": np.array([0, 0, 2])}
