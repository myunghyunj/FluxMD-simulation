...
@pytest.mark.skipif(True, reason="This test is redundant with test_parallel.py and was causing issues.")
@patch.object(BrownianSurfaceRoller, 'run', _dummy_run)
def test_parallel_execution_smoke_test():
    ...
    """
    protein_atoms = {
        'coords': np.random.rand(50, 3), 'names': np.array(['CA']*50),
        'radii': np.ones(50), 'masses': np.ones(50)*12, 'resnames': np.array(['ALA']*50)
    }
    ligand_atoms = {
        'coords': np.random.rand(5, 3), 'names': np.array(['C']*5), 'masses': np.ones(5)*12
    }
    params = {'n_workers': 2, 'use_ref15': False}
