...
from tests.test_matryoshka_integration import create_synthetic_protein, create_synthetic_ligand
...
class PhysicsValidator:
    ...
    def validate_diffusion_coefficient(self) -> Dict:
        ...
        for lag in range(1, max_lag):
            ...
            tangential_disp = displacements - np.sum(displacements * radial, axis=1, keepdims=True) * radial
    ...
    def validate_boltzmann_statistics(self) -> Dict:
        ...
        ax.axhline(y=n_steps/2, color='r', linestyle='--', alpha=0.5, 
                  label=f'Equal occupancy')
        
        # Add expected ratio line
        expected_counts = [n_steps * np.exp(-0) / (np.exp(-0) + np.exp(-delta_E)),
                          n_steps * np.exp(-delta_E) / (np.exp(-0) + np.exp(-delta_E))]
        ax.plot([0, 1], expected_counts, 'go-', linewidth=2, markersize=10,
        ...
        ax.text(0.5, 0.95, f'Measured ratio: {ratio_measured:.2f}',
...
def main():
    ...
    parser.add_argument("--output-dir", type=Path, default="validation_results",
