...
class TestREF15ZeroVolume:
    ...
    def test_zero_volume_warning(self):
        ...
        atom1 = AtomContext(atom_type="C", coords=np.array([0.0, 0.0, 0.0]), neighbor_count=0)
        atom2 = AtomContext(atom_type="N", coords=np.array([3.0, 0.0, 0.0]), neighbor_count=0)
    ...
    def test_both_zero_volumes(self):
        ...
        atom1 = AtomContext(atom_type="X", coords=np.array([0.0, 0.0, 0.0]), neighbor_count=0)
        atom2 = AtomContext(atom_type="Y", coords=np.array([3.0, 0.0, 0.0]), neighbor_count=0)
    ...
    def test_normal_volumes_no_warning(self):
        ...
        # Mock normal volumes
        with patch.object(calc.params, "get_lk_params") as mock_get_lk:
            mock_get_lk.side_effect = [(1.0, 3.5, 10.0), (1.0, 3.5, 15.0)]  # Normal volumes
            ...
            with warnings.catch_warnings(record=True) as w:
                ...
                # Should not warn
                volume_warnings = [
                    warning for warning in w if "Zero atomic volume" in str(warning.message)
                ]
    ...
    def test_parameter_validation_logging(self):
        ...
        with patch("fluxmd.core.ref15_energy.get_ref15_params", return_value=mock_params):
            with patch("fluxmd.core.ref15_energy.logger") as mock_logger:
                ...
                volume_warnings = [
                    call for call in warning_calls if "zero volume" in str(call).lower()
                ]
                assert len(volume_warnings) > 0
