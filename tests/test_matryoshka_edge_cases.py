...
class TestSurfaceEdgeCases:
    ...
    def test_self_intersecting_surface(self):
        ...
        vertices = np.array([
            [0, 0, 0], [2, 0, 0], [1, 0.1, 0],  # Nearly colinear triangle
            [1, 0, 1], [1, 0, -1]  # Points above and below
        ])
        faces = np.array([
            [0, 1, 2], [0, 2, 3], [1, 2, 4]
        ])
    ...
    def test_surface_with_holes(self):
        ...
        faces = np.array([
            [0, 1, 4], [1, 2, 4],  # Two faces only, creating a hole
        ])
    ...
    def test_extreme_layer_offsets(self):
        ...
        assert all(abs(offset - expected_offset) / expected_offset < 0.1 
...
class TestEnergyCalculationEdgeCases:
    ...
    def test_overlapping_atoms(self):
        ...
        atom2 = AtomContext(
            atom_type='C',
            coords=np.array([0.001, 0, 0]),  # Nearly overlapping
            formal_charge=0.0
        )
