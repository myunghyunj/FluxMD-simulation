...
def calculate_ribbon_path(backbone_data, smooth_factor=10):
    ...
    try:
        tck, u = splprep([ca_coords[:, 0], ca_coords[:, 1], ca_coords[:, 2]], 
                         s=smooth_factor, k=min(3, len(ca_coords)-1))
...
def plot_ribbon_protein(ax, pdb_file, flux_df, title):
    ...
    if not backbone_data:
        ax.text(0.5, 0.5, 0.5, f"No protein data found in\n{os.path.basename(pdb_file)}", 
    ...
    for chain_id, chain_data in backbone_data.items():
        ...
        flux_interp = np.interp(np.linspace(0, len(norm_flux)-1, len(ribbon_path)-1),
                               np.arange(len(norm_flux)), norm_flux)
        ...
        lc = Line3DCollection(segments, colors=colors, linewidths=4, 
                             alpha=0.9, capstyle='round', joinstyle='round')
        ...
        for offset in [-0.5, 0.5]:
            ...
            lc_offset = Line3DCollection(segments_offset, colors=colors, 
    ...
    ax.text2D(0.5, 0.02, f'Flux range: {flux_min:.2f} - {flux_max:.2f}', 
    ...
    if save_individual == 'y':
        for idx, (pdb_file, csv_file, label) in enumerate(protein_flux_pairs):
            ...
            if flux_df is not None:
                ...
                sm = cm.ScalarMappable(cmap=berlin_cmap)
                sm.set_array([])
                cbar = fig_single.colorbar(sm, ax=ax_single, orientation='vertical', 
                                         pad=0.1, fraction=0.03)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        ...
        parser = argparse.ArgumentParser(description='Visualize multiple proteins as ribbons colored by flux values')
        parser.add_argument('--proteins', nargs='+', required=True,
                          help='PDB files for proteins')
        parser.add_argument('--fluxes', nargs='+', required=True,
                          help='CSV files with flux data')
        parser.add_argument('--labels', nargs='+', required=True,
                          help='Labels for each protein')
        parser.add_argument('--output', default='multiflux_comparison.png',
